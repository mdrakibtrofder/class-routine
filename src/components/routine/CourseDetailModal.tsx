...
export const CourseDetailModal = ({ session, open, onOpenChange }: CourseDetailModalProps) => {
  const { getCourseByCode, getTeacherByCode, getDepartmentByCode } = useRoutineData();
      ...
      <DialogContent className="max-w-lg bg-card border-border shadow-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${course?.type === 'sessional' ? 'bg-gradient-secondary' : 'bg-gradient-primary'}`}>
              {course?.type === 'sessional' ? <FlaskConical className="w-6 h-6 text-white" /> : <BookOpen className="w-6 h-6 text-white" />}
            </div>
            <div>
        ...
        <div className="space-y-6 pt-4">
          ...
          <div className="space-y-3">
            <h4 className="text-sm font-medium text-muted-foreground flex items-center gap-2"><Users className="w-4 h-4" /> Instructors</h4>
            <div className="space-y-2">
              {teachers.map(teacher => teacher && (
                <div key={teacher.code} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                  <div className="w-10 h-10 rounded-full bg-gradient-primary flex items-center justify-center text-white font-bold text-sm">{teacher.code}</div>
                  <div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium border bg-gradient-to-r from-blue-500/20 to-indigo-500/20 text-blue-700 border-blue-300">
                <GraduationCap className="w-4 h-4" /><span className="text-xs opacity-70">Year -</span><span className="font-semibold">{session.year}</span>
              </div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium border bg-gradient-to-r from-emerald-500/20 to-teal-500/20 text-emerald-700 border-emerald-300">
                <Calendar className="w-4 h-4" /><span className="text-xs opacity-70">Semester -</span><span className="font-semibold">{session.semester}</span>
              </div>
              {session.section && (
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium border bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-purple-700 border-purple-300">
                  <LayoutGrid className="w-4 h-4" /><span className="text-xs opacity-70">Section -</span><span className="font-semibold">{session.section}</span>
                </div>
              )}
              {session.isOddRoll && (
          ...
          {course && (
            <div className="grid grid-cols-3 gap-3 pt-2">
              <div className="text-center p-3 rounded-xl bg-primary/10">
                <CreditCard className="w-5 h-5 mx-auto mb-1 text-primary" /><p className="text-xs text-muted-foreground">Credits</p><p className="font-bold text-primary">{course.creditHours}</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-muted/50">
                <BookOpen className="w-5 h-5 mx-auto mb-1 text-muted-foreground" /><p className="text-xs text-muted-foreground">Theory</p><p className="font-bold">{course.theoryHours}h</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-secondary/10">
                <FlaskConical className="w-5 h-5 mx-auto mb-1 text-secondary" /><p className="text-xs text-muted-foreground">Lab</p><p className="font-bold text-secondary">{course.sessionalHours}h</p>
              </div>
            </div>
          )}
        </div>
